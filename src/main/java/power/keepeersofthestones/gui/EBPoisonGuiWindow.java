...
@OnlyIn(Dist.CLIENT)
public class EBPoisonGuiWindow extends ContainerScreen<EBPoisonGui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	private final static HashMap guistate = EBPoisonGui.guistate;

	public EBPoisonGuiWindow(EBPoisonGui.GuiContainerMod container, PlayerInventory inventory, ITextComponent text) {
	...
	protected void drawGuiContainerBackgroundLayer(MatrixStack ms, float partialTicks, int gx, int gy) {
		...
		Minecraft.getInstance().getTextureManager().bindTexture(new ResourceLocation("power:textures/screens/poison_stone.png"));
	...
	protected void drawGuiContainerForegroundLayer(MatrixStack ms, int mouseX, int mouseY) {
		...
		this.font.drawString(ms, "Element: Poison", -162, -67, -12829636);
		this.font.drawString(ms, "Force: Harm", -162, -49, -12829636);
	}
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + 152, this.guiTop + 86, 30, 20, new StringTextComponent(">"), e -> {
			if (true) {
				PowerMod.PACKET_HANDLER.sendToServer(new EBPoisonGui.ButtonPressedMessage(0, x, y, z));
				EBPoisonGui.handleButtonAction(entity, 0, x, y, z);
			}
		}));
	}
}
