...
@OnlyIn(Dist.CLIENT)
public class EBSpeedGuiWindow extends ContainerScreen<EBSpeedGui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	private final static HashMap guistate = EBSpeedGui.guistate;

	public EBSpeedGuiWindow(EBSpeedGui.GuiContainerMod container, PlayerInventory inventory, ITextComponent text) {
	...
	protected void drawGuiContainerBackgroundLayer(MatrixStack ms, float partialTicks, int gx, int gy) {
		...
		Minecraft.getInstance().getTextureManager().bindTexture(new ResourceLocation("power:textures/screens/speed_stone.png"));
	...
	protected void drawGuiContainerForegroundLayer(MatrixStack ms, int mouseX, int mouseY) {
		...
		this.font.drawString(ms, "Element: Speed", -162, -67, -12829636);
		this.font.drawString(ms, "Force: Boost", -162, -49, -12829636);
	}
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + 152, this.guiTop + 86, 30, 20, new StringTextComponent(">"), e -> {
			if (true) {
				PowerMod.PACKET_HANDLER.sendToServer(new EBSpeedGui.ButtonPressedMessage(0, x, y, z));
				EBSpeedGui.handleButtonAction(entity, 0, x, y, z);
			}
		}));
	}
}
