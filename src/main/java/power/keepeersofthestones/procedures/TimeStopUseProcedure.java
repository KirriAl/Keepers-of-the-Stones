	...
	public static void executeProcedure(Map<String, Object> dependencies) {
		...
		if (dependencies.get("sourceentity") == null) {
			if (!dependencies.containsKey("sourceentity"))
				PowerMod.LOGGER.warn("Failed to load dependency sourceentity for procedure TimeStopUse!");
			return;
		}
					...
					.orElse(new PowerModVariables.PlayerVariables())).recharge_spell_time) {
				{
					...
					if (!_ent.world.isRemote && _ent.world.getServer() != null) {
						_ent.world.getServer().getCommandManager()
								.handleCommand(_ent.getCommandSource().withFeedbackDisabled().withPermissionLevel(4), "scale set pehkui:motion 0 @s");
					}
				}
				new Object() {
					private int ticks = 0;
					...
					private void run() {
						{
							...
							if (!_ent.world.isRemote && _ent.world.getServer() != null) {
								_ent.world.getServer().getCommandManager().handleCommand(
										_ent.getCommandSource().withFeedbackDisabled().withPermissionLevel(4), "scale set pehkui:motion 1 @s");
							}
						}
